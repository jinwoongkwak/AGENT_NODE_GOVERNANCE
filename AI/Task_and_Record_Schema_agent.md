...
AI 작업 문서가 가져야 할 종류, frontmatter 필드, 본문 구획, 문서 사이 규칙을 정의합니다. 문서가 어디에 쓰이는지는 [문서 체계](../Architecture/Document_System_admin.md#작업-문서)에 있습니다.
...
| 본문 구획 | 종류별 필수 `##` 제목 | evaluation은 입력·[필수 조건](Roles/Evaluator_agent.md#필수-조건)·점수·발견·판정 근거 | 근거 없이 판정만 남음 |
| 불변식 | 문서 사이의 규칙 | 실행한 plan에는 pass 평가가 있어야 함 | 평가받지 않은 계획이 실행됨 |

| 용어 | 뜻 | 파일 |
|---|---|---|
| 스키마 | 규칙: 무엇이 올바른 문서인가 | 이 문서 |
| 템플릿 | 규칙을 만족하는 빈 틀 | [`{templates}`](../Architecture/Company_Profile_admin.md#작업-공간-경로)의 작업·교환 기록 템플릿 |
| 검사기 | 규칙대로 문서를 검사하는 프로그램 | [Router](Routing_agent.md#router의-역할)의 `check`와 `new-record` |
...
현재 AI 작업 문서는 대표 task 한 종류, 곧 [TaskNote](../Architecture/Document_System_admin.md#작업-문서)입니다.
...
대표 task는 파일 이름이 제목이자 ID입니다 ([명명 규칙](../Architecture/Company_Profile_admin.md#명명-규칙)).
...
대표 task의 `proposal_version`과 `approved_version`은 `V<major>.<minor>.<patch>` 형식입니다. 버전을 올리는 기준은 [버전 규칙](../HQ/Commands_and_Approval_admin.md#버전-규칙)에만 정의합니다.
...
이 블록은 **확장 모드의 스키마**이며 Router 구현 전에는 실행 가능한 검사기라고 간주하지 않습니다. 기본 모드는 교환 기록을 쓰지 않으며 기존 task는 legacy로 읽습니다. `blocked_by`의 기존 외부 조건 문장은 본문으로 보존하고, `blockedBy`에는 task 링크 목록만 둡니다. Router가 구현되면 이 블록을 읽어 검사합니다. 위의 표와 블록이 다르면 `check-schema`가 실패합니다. `{hq-owner}`는 Router가 [회사 프로필](../Architecture/Company_Profile_admin.md#사람과-역할-배정)의 값으로 바꿔 읽습니다.
...
저장소 업데이트와 회사의 운영 버전 갱신은 별개입니다 ([버전 고정](../Setup/Adoption_admin.md#버전-구분)).

## 관련-문서

- [라우팅](Routing_agent.md) — 이 스키마를 검사하고 파일을 만드는 Router
- [문서 체계](../Architecture/Document_System_admin.md) — 문서가 어디에 쓰이는지
- [기록 형식](Reporting_Style_agent.md) — `# 기록` 본문의 형식
- [작업 흐름](Workflow_agent.md#단계별-기록) — 단계마다 만드는 기록
