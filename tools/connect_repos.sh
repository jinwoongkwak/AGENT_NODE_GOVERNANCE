...
Usage: connect_repos.sh --root DIR --manifest FILE [options]
  ...
  --install-launchers   With --apply, write Connect_Repos.sh at the root and a standalone
                        Connect_Repo.sh in every existing repository folder. The standalone
                        launcher embeds its remote, branch, config, and the connect logic,
                        so it needs neither the root launcher, this tool, nor the manifest.
...
# Connects or checks one repository folder. Standalone launchers embed this and LAUNCHER_FUNCS.
connect_one() {
  local dir="$1" path="$2" remote="$3" branch="$4" config="$5"
...
# Everything connect_one needs at run time. A standalone launcher copies these with declare -f.
LAUNCHER_FUNCS=(report to_win dropbox_marks ensure_exclude apply_config submodule_paths
  submodule_pointer_files restore_submodules state_summary connect_new check_existing connect_one)

repo_launcher() {
  local path="$1" remote="$2" branch="$3" config="$4" tool_rel="$5" manifest_rel="$6"
  printf '%s\n' '#!/usr/bin/env bash' \
    "# Double-click before work: reconnect this folder's repository to its remote. Needs only Git Bash." \
    '# Standalone: it does not read the root launcher, the tool, or the list. It fetches but never pulls or pushes.' \
    "# Generated by $tool_rel --install-launchers from $manifest_rel. Change the list and regenerate; do not edit." \
    'set -u' ''
  printf 'REPO_PATH=%q\nREPO_REMOTE=%q\nREPO_BRANCH=%q\nREPO_CONFIG=%q\n' "$path" "$remote" "$branch" "$config"
  printf 'LAUNCHER_NAME=%q\nMARK_STREAM=%q\nAPPLY=1\n' "$LAUNCHER_NAME" "$MARK_STREAM"
  printf '%s\n\n' 'N_OK=0; N_CONNECTED=0; N_INFO=0; N_WARN=0; N_SKIP=0; N_FAIL=0'
  declare -f "${LAUNCHER_FUNCS[@]}"
  cat <<'EOF'

here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
connect_one "$here" "$REPO_PATH" "$REPO_REMOTE" "$REPO_BRANCH" "$REPO_CONFIG"
status=0; [ "$N_FAIL" -eq 0 ] || status=1
...
install_launchers() {
  local tool_rel manifest_rel root_content path remote branch config dir
...
exit \$status"
  ...
  while IFS=$'\t' read -r path remote branch config || [ -n "${path:-}" ]; do
    ...
    if [ -z "$remote" ] || [ -z "$branch" ]; then
      echo "launcher skipped: $path (목록에 remote 또는 branch가 비어 있음)"
      continue
    fi
    write_if_changed "$dir/$LAUNCHER_NAME" \
      "$(repo_launcher "$path" "$remote" "$branch" "$config" "$tool_rel" "$manifest_rel")"
...
while IFS=$'\t' read -r path remote branch config || [ -n "${path:-}" ]; do
  ...
  connect_one "$ROOT/$path" "$path" "$remote" "$branch" "$config"
done < "$MANIFEST"
