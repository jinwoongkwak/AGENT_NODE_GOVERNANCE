...
[HQ](Operating_Model.md#hq와-ai의-뜻)와 AI Agent 사이에서 지시·결정·실행 지시·보고가 오가는 길을 설명합니다. 모든 전달은 작업마다 하나인 [TaskNote](Document_System.md#작업-문서)에 모입니다.

| 섹션 | 내용 | 상태 | 근거 |
|---|---|---|---|
| [전달-체계-한눈에](#전달-체계-한눈에) | 전체 흐름도와 전달물 표 | adopted | DEC-HQ-005 |
| [지시-흐름](#지시-흐름) | HQ 요청이 정리된 지시가 되기까지 | adopted | DEC-HQ-005, 기존 Handoff §2 |
| [결정과-승인-흐름](#결정과-승인-흐름) | 결정 요청, HQ 결정, 승인 기록 | adopted | DEC-HQ-005 |
| [실행-지시-흐름](#실행-지시-흐름) | 승인 후 실행을 시작하고 멈추는 법 | adopted | DEC-HQ-005 |
| [보고-흐름](#보고-흐름) | 기록, 정본 반영, 후속 제안 | adopted, proposed | DEC-HQ-005, 제안서 05 |
| [작업-상태](#작업-상태) | 상태 조합 6개와 전이 | adopted, proposed | 기존 Handoff §1, 제안서 05 Q1 |
| [교환-기록-흐름](#교환-기록-흐름) | 역할 사이 기록 전달 | proposed | 제안서 01·03 |
| [관련-문서](#관련-문서) | 명령·승인의 상세 문서 | — | — |
...
| AI → HQ | 결정 요청 | TaskNote `# 결정 및 승인`의 [결정표](../HQ/Commands_and_Approval.md#결정표-작성) |
...
| 3 | AI Agent | 가장 가까운 [CONTEXT](Document_System.md#정본-문서)와 정본을 읽고, [위험도](Risk_and_Authority.md#위험도), 의존성, 백업, [쓰기 범위](../HQ/Control_Settings.md#쓰기-범위) 겹침을 확인 | `# 현재 상태`가 최신 |
...
지시를 [작업 계약](../AI/Roles/Coordinator.md#계약-정규화)으로 고정하는 제안된 세부 조항은 Coordinator의 계약 정규화 조항(CO-111–CO-114)에 있습니다.
...
- **승인과 실행은 다릅니다.** [실행 모드](Risk_and_Authority.md#실행-모드)가 manual이면 승인은 계획만 기록하고, HQ가 `<제목> 실행해`를 보내야 실행이 시작됩니다.
...
| 단계 | 할 일 | 정의 |
|---|---|---|
| 기록 | `# 현재 상태`를 갱신하고 버전 붙은 기록 항목을 `# 기록`에 추가 | [기록 구조](../AI/Reporting_Style.md#기록-구조) |
| 알림 | 작업의 보고 정책에 맞춰 HQ에게 알림 | [보고 정책](../HQ/Control_Settings.md#보고-정책) |
| 정본 반영 | 결과가 STATUS·Decisions·Wiki에 속하면 반영 | [정본 승격 확인](../HQ/Review_and_Closure.md#정본-승격-확인) |
| 후속 제안 | 미해결이나 인계가 남으면 중복 없는 다음 버전 제안을 즉시 작성 | [후속 제안 처리](../HQ/Review_and_Closure.md#후속-제안-처리) |
| 종료 | 원래 완료 기준과 검증을 모두 통과했을 때만 닫음 | [종료·취소·보류](../HQ/Review_and_Closure.md#종료-취소-보류) |
...
내부 반복(계획 수정, 재평가, 한도 안의 재시도)은 모두 기록하지만 HQ에게는 알리지 않습니다. HQ에게는 결정 필요, 실패·중단, 완료, 계약에 적힌 milestone만 알립니다 ([HQ 보고 시점](../AI/Roles/Coordinator.md#hq-보고-시점)).
...
[과정 역할](Organization.md#과정-역할)을 도입하면 역할 사이의 전달물이 [교환 기록](Document_System.md#교환-기록)으로 남습니다. 역할은 본문만 돌려주고, Coordinator가 [Router](../AI/Routing.md#router의-역할)로 기록을 저장하며 대표 노트에 링크를 추가합니다.
...
단계마다 어떤 기록을 누가 만드는지는 [단계별 기록](../AI/Workflow.md#단계별-기록)에 있습니다.

## 관련-문서

- [명령과 승인](../HQ/Commands_and_Approval.md) — HQ가 쓰는 채팅 명령과 결정표
- [작업 흐름](../AI/Workflow.md) — AI 쪽에서 본 같은 흐름
- [위험도와 권한](Risk_and_Authority.md) — 결정과 승인이 필요한 경계
- [문서 체계](Document_System.md) — TaskNote의 구조
