...
AI 작업 파일을 어디에, 어떤 이름으로, 어떤 검사를 거쳐 만드는지 정합니다. 현재는 규칙만 있고, 제안된 Router 프로그램이 승인되면 그 규칙을 기계로 적용합니다.

| 섹션 | 내용 | 상태 | 근거 |
|---|---|---|---|
| [router의-역할](#router의-역할) | Router가 하는 일과 하지 않는 일, 구성, 호출 흐름 | proposed | 제안서 03 Q1 |
| [배치-규칙](#배치-규칙) | TaskNote와 [교환 기록](../Architecture/Document_System.md#교환-기록)의 위치 | adopted, proposed | DEC-HQ-005, 제안서 03 (V1.0.2 Q5 이월) |
| [프로젝트-판정](#프로젝트-판정) | 저장할 프로젝트를 정하는 근거 | proposed | 제안서 03 Q3 |
| [명령](#명령) | Router 명령과 종료 코드 | proposed | 제안서 03 |
| [처리-순서](#처리-순서) | 파일 생성의 단계와 frontmatter 처리 | proposed | 제안서 03 |
| [실패와-동시성](#실패와-동시성) | 실패 상황의 처리와 시험 | proposed | 제안서 03 |
| [관련-문서](#관련-문서) | 스키마와 작업 공간 | — | — |
...
| [스키마](Task_and_Record_Schema.md#기계-판독-블록)와 [불변식](Task_and_Record_Schema.md#불변식) 검사, 위반 시 저장 거부 | 다음에 어떤 역할을 부를지 결정 |
| 부모 노트에 기록 링크 한 줄 추가 | 기존 파일 이동·삭제 (이동표만 출력) |
| 작업 잠금 파일 관리 | 상시 감시, 자동 재개 |

| 구성 | 위치 | 버전 관리 | 내용 |
|---|---|---|---|
| Router 프로그램 | [`{router}`](../Architecture/Company_Profile.md#작업-공간-경로) | 추적 | 명령줄 프로그램 한 파일. 스크립트 런타임의 표준 라이브러리만 사용 |
| 시험 | Router와 같은 폴더의 시험 파일 | 추적 | 임시 폴더의 가짜 작업 공간에서 시험 |
| 스키마 | [작업과 기록 스키마](Task_and_Record_Schema.md#기계-판독-블록) | 추적 | Router가 읽는 json 블록 |
| 프로젝트 키 표 | [프로젝트 키](../Architecture/Company_Profile.md#프로젝트-키) | 추적 | Router가 읽는 표 |
| 런타임 폴더 | [`{runtime-dir}`](../Architecture/Company_Profile.md#작업-공간-경로) | 제외, 동기화 밖 | `staging/<task_id>/` 역할 산출물 본문, `locks/<task_id>.lock` |
...
현재 모든 AI TaskNote는 [`{task-folder}`](../Architecture/Company_Profile.md#작업-공간-경로) 바로 아래에 두고, 파일 이름이 제목이자 ID입니다. 상태에 따라 폴더를 옮기지 않습니다.

### 프로젝트-폴더-배치-제안

> [!NOTE]
> 제안 — 제안서 03 (V1.0.2 Q5 입력 이월) 결정 대기.

| 규칙 | 내용 |
|---|---|
| 대표 task | `<task-folder>/<저장 폴더>/<제목>.md`. 저장 폴더는 [프로젝트 키](../Architecture/Company_Profile.md#프로젝트-키) 표에 있음 |
| 교환 기록 | 같은 저장 폴더의 `R/` |
...
| 경로 길이 | 절대 경로 240자 이하 |
| 상태 폴더 | 만들지 않음 |
| 이동 | Router는 옮기지 않음. `plan-moves`가 이동표만 출력하고 이동은 [위험도](../Architecture/Risk_and_Authority.md#위험도) 2 승인 후 `git mv` |
...
1. **후보 키:** Coordinator가 넘긴 `--project`. HQ 원문에 적힌 키만 넘깁니다 ([접수](Roles/Coordinator.md#접수)).
...
| # | 단계 | 실패하면 |
|---:|---|---|
| 1 | 인자 확인: 제목에 금지 접두사 없음 ([명명 규칙](../Architecture/Company_Profile.md#명명-규칙)) | 1 |
| 2 | [프로젝트 판정](#프로젝트-판정) | 3 |
| 3 | 기밀 경로 검사 | 3 |
...
| lease가 지난 잠금 | 자동으로 빼앗지 않음. Coordinator가 프로세스를 확인한 뒤 `unlock --force --reason` ([잠금과 예산](Roles/Coordinator.md#잠금과-예산)) |
...
- [작업과 기록 스키마](Task_and_Record_Schema.md) — Router가 검사하는 규칙
- [작업 공간과 도구](../Architecture/Workspace_and_Tools.md#런타임과-router) — Router가 속한 런타임 계층
- [Coordinator](Roles/Coordinator.md) — Router를 호출하는 역할
- [회사 프로필](../Architecture/Company_Profile.md#프로젝트-키) — 프로젝트 키 표
