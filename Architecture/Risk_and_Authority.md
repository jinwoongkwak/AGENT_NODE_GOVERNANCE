...
어떤 일을 AI가 스스로 하고 어떤 일에 [HQ](Operating_Model.md#hq와-ai의-뜻)의 승인이 필요한지 정의합니다. 위험도, 실행 모드, 검토 깊이, 판단 경계는 이 문서에서만 정의하고 다른 문서는 여기로 링크합니다.

| 섹션 | 내용 | 상태 | 근거 |
|---|---|---|---|
| [위험도](#위험도) | 0–2 등급과 필요한 문서 구조 | adopted | DEC-HQ-001, 기존 AI_Protocol §5 |
| [실행-모드](#실행-모드) | autonomous, after-approval, manual | adopted | DEC-HQ-005 |
| [검토-깊이](#검토-깊이) | 경량, 표준, 엄격 | proposed | 제안서 04 (V1.0.2 Q2 이월) |
| [판단-권한-경계](#판단-권한-경계) | AI가 맡는 판단과 HQ가 맡는 판단 | adopted, proposed | DEC-HQ-001, 제안서 05 (V1.0.2 Q4 이월) |
| [위임하지-않는-행위](#위임하지-않는-행위) | 누구에게도 자동으로 맡기지 않는 행위 | adopted | DEC-HQ-002, DEC-HQ-005 |
| [관련-문서](#관련-문서) | 설정과 확인 절차 | — | — |
...
- **manual의 뜻:** manual 작업은 승인이 실행을 뜻하지 않습니다 ([실행 지시 흐름](Command_and_Report_Flow.md#실행-지시-흐름)).

## 검토-깊이

> [!NOTE]
> 제안 — 제안서 04 (V1.0.2 Q2 입력 이월) 결정 대기. 현재는 한 Agent가 계획·실행·자체 확인을 모두 합니다.
...
접수할 때 정하고, 진행 중에는 올리기만 할 수 있습니다 ([검토 깊이와 위험도](../AI/Roles/Coordinator.md#검토-깊이와-위험도)).
...
HQ가 미리 위임한 선택은 [작업 계약](../AI/Roles/Coordinator.md#계약-정규화)에 적고 반복해서 승인받지 않습니다. 위임 범위가 없는 결정은 평가 점수로 대신하지 않습니다.
...
| 기밀 원문을 외부 AI 서비스로 전송 | 누구도 하지 않음 | 기밀 유지 의무 ([기밀](../AI/Common_Rules.md#기밀)) |
| 원 데이터·원 보고서·EDA DB·제출 논문 덮어쓰기 | 누구도 하지 않음 | 증거 손실 |

## 관련-문서

- [제어 설정](../HQ/Control_Settings.md) — HQ가 작업마다 위험도와 모드를 고르는 법
- [공통 규칙](../AI/Common_Rules.md) — AI가 실행 전에 지키는 파일·백업·기밀 규칙
- [명령과 보고 체계](Command_and_Report_Flow.md) — 승인과 실행 지시가 오가는 흐름
- [운영 모델](Operating_Model.md) — "안전은 장치로" 원칙
