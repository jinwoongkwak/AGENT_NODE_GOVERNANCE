...
PROTOCOL_PATH = '00_HQ/90_SYSTEM/JM_Protocol'
...
2. `00_HQ/90_SYSTEM/Company_Profile.md`와 `{PROTOCOL_PATH}/AI/README.md`를 읽습니다.
3. 해당 TaskNote·작업 영역의 `_AI/CONTEXT.md`·원 증거를 읽습니다.
4. 승인된 범위에서 백업·실행·검증·기록합니다. 기존 대화의 명시 권한은 다시 묻지 않습니다.
5. 기밀 원문은 명시 범위에서만 열고 외부 AI에 보내지 않습니다. 영구 삭제 금지, Git은 ai/work, main 병합은 HQ입니다.
6. draft 작업 공간은 설치·분류 계획만 준비합니다. 실제 운영은 HQ 승인·검증·채택 기록 후 활성화합니다.
'''
    put('AGENTS.md', entry)
    put('CLAUDE.md', entry)
...
| 운영 모드 | basic |
| 프로토콜 | [{VERSION} 매뉴얼](JM_Protocol/README.md) |
| TaskNote | `{TASKS}/` |
| 기밀·제한 자료 | 도입 전에 HQ가 경로와 허용 처리 환경을 확인해야 함 |
| Git·백업·실행 기기 | 도입 시 실제 환경에서 지정·검증 |
| 자료 배치 | [배치 기준](JM_Protocol/Setup/Material_Placement.md) |
''')
...
- [관리자 운영 매뉴얼](90_SYSTEM/JM_Protocol/HQ/Operating_Manual.md)
...
''')
    ...
    put('00_HQ/_AI/CONTEXT.md', '# HQ Context\n\n정본: `../Project_Index.md`, `../Decisions.md`, TaskNote. 공통 규칙은 `../90_SYSTEM/JM_Protocol/AI/README.md`, 회사 값은 `../90_SYSTEM/Company_Profile.md`.\n')
