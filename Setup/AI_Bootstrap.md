...
2. [작업 공간 구조](../Architecture/Workspace_Layout.md#최소-구조)에 맞춘 파일 생성 목록을 만듭니다. 기존 작업 공간이면 [이전 절차](Migration.md)를 사용합니다.
3. 새 빈 작업 공간을 만들고 프로필·진입 파일·CONTEXT·프로젝트 정본·작업 템플릿·HQ 행동 뷰를 생성합니다.
4. [자료 배치 기준](Material_Placement.md)에 따라 각 입력을 파일별 대응표에 분류합니다. 원본·Git repo·기밀 자료를 일반 문서와 같은 방식으로 이동하지 않습니다.
5. 허용된 범위의 파일 배치만 실행합니다. 모르는 자료는 원위치 또는 INBOX에 두고 질문·사유를 남깁니다.
6. 프로토콜 검사·구조 검사·합성 작업·복구 검증을 통과시킨 뒤 HQ의 실제 승인으로 채택 기록을 활성화합니다.

## 생성-도구

[예제 설정](company.example.json)을 새 회사 값으로 바꾼 뒤 다음 명령을 사용합니다. 회사 이름·사람·프로젝트만 받으며, 승인이나 기밀 정책을 예제에서 상속하지 않습니다.
...
- [자료 배치 기준](Material_Placement.md) — 입력을 어디에 둘지 결정
- [초기 템플릿](Templates.md) — 수동 생성 대안
- [관리자 운영 매뉴얼](../HQ/Operating_Manual.md) — 설립 뒤 운영
- [회사 프로필](../Architecture/Company_Profile.md) — 로컬 설정 계약
