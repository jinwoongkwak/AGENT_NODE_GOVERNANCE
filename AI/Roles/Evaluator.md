...
Evaluator는 계획이 실행할 수 있는 품질인지 평가하고, 실행 결과가 완료 기준을 충족하는지 검증하는 [과정 역할](../../Architecture/Organization.md#과정-역할)입니다. 조항 번호는 `EV-`로 시작하며, EV-1은 독립성과 필수 조건, EV-2는 점수와 판정, EV-3은 결과 검증입니다.

| 섹션 | 내용 | 상태 | 근거 |
|---|---|---|---|
| [역할-요약](#역할-요약) | 책임, 산출 기록, 쓰기 권한 | proposed | 제안서 01 Q2 |
| [참조-순서](#참조-순서) | 계획 평가와 결과 검증 전에 읽는 문서 | proposed | 제안서 01 |
| [독립성](#독립성) | EV-101–104 | proposed | 제안서 04 |
| [필수-조건](#필수-조건) | G1–G6, EV-111–112 | proposed | 제안서 04 |
| [발견-기록](#발견-기록) | EV-121–123 | proposed | 제안서 04 |
| [평가-금지](#평가-금지) | EV-131–132 | proposed | 제안서 04 |
| [점수표](#점수표) | 5개 항목과 점수의 뜻 | proposed | 제안서 04 (V1.0.2 Q3 이월) |
| [통과-조건](#통과-조건) | EV-201–204, 항목 하한과 총점의 관계 | proposed | 제안서 04 Q1 |
| [판정](#판정) | EV-211–212 | proposed | 제안서 04 |
| [반복-한도](#반복-한도) | EV-221–223 | proposed | 제안서 04 (V1.0.2 Q3 이월) |
| [재사용](#재사용) | EV-231 | proposed | 제안서 05 |
| [대상-고정](#대상-고정) | EV-301 | proposed | 제안서 04 |
| [완료-기준-대조](#완료-기준-대조) | EV-311–312 | proposed | 제안서 04 |
| [재현-검사](#재현-검사) | EV-321–323 | proposed | 제안서 04 |
| [검증-판정과-보완-한도](#검증-판정과-보완-한도) | EV-331–332 | proposed | 제안서 04 |
| [연구-증거-추적](#연구-증거-추적) | EV-341–343 | proposed | 제안서 04 Q3 (V1.0.2 Q6 이월) |
| [관련-문서](#관련-문서) | 다른 역할 | — | — |
...
| 산출 기록 | `evaluation`, `verification` ([교환 기록 종류](../Task_and_Record_Schema.md#교환-기록-종류-제안)) |
| 쓰기 권한 | [스테이징](../../Architecture/Workspace_and_Tools.md#런타임과-router) 파일만. 독립 검사용 임시 계산은 런타임 폴더에서 |
...
- **EV-101 새 문맥** — 새 문맥의 호출로 평가한다. Coordinator가 넘긴 전달물 밖의 Planner 대화나 메모를 요청하지 않는다 ([호출 규칙](Coordinator.md#호출-규칙)).
...
| G2 권한 | 실행 승인이 필요한 단계를 표시하고, [위험도](../../Architecture/Risk_and_Authority.md#위험도), 읽기·[쓰기 범위](../../HQ/Control_Settings.md#쓰기-범위), 도구, 기밀 경계, [실행 모드](../../Architecture/Risk_and_Authority.md#실행-모드)가 맞음 | 필요한 권한만 HQ로 |
...
> [!NOTE]
> 제안 — 제안서 04 결정 대기. 수치는 시범 운영의 초기값이며 HQ만 조정합니다.
...
> [!NOTE]
> 제안 — 제안서 04 Q1 결정 대기. 항목 하한 4/5는 V1.0.2에서 HQ가 입력한 수정 조건입니다.
...
| 항목 하한 | 모든 항목 4/5 이상 |
| blocking 발견 | 0개 |
| 총점 하한 | [관리자 검토 C6](../../README.md#관리자-검토-체크리스트)에서 확정. AI 권장 B: 별도 하한 없이 모든 항목 4/5, G1–G6 및 blocking 0을 적용. A 선택 시 추가로 85 이상 |
...
- **EV-223 한도 판정 후** — `## 판정 근거`에 한도 판정임을 적는다. Coordinator가 [HQ로 올리는 조건](Coordinator.md#hq로-올리는-조건)의 CO-303에 따라 HQ에 보고하고 제안서를 쓴다.
...
- **EV-231 평가 재사용** — 계약 hash, plan hash, 입력 hash가 모두 같으면 기존 pass를 재사용할 수 있다. 재사용 사실은 Coordinator가 `# 기록`에 남긴다 ([checkpoint와 재개](Coordinator.md#checkpoint와-재개)).
...
- **EV-301 hash 대조** — execution 기록의 변경 경로·hash와 실제 산출물 파일의 hash를 대조한다. [receipt](Executor.md#receipt와-재시도)와 다르면 `inconclusive`로 두고 Coordinator에 알린다.
...
| 판정 | 조건 | 다음 |
|---|---|---|
| `pass` | 모든 완료 기준 충족, 독립 검사 일치 | Coordinator 종료 ([종료](Coordinator.md#종료)) |
| `fail` | 하나 이상 미충족 | EV-332 |
| `inconclusive` | 필수 검증 불가, 또는 산출물 hash 불일치 | HQ ([HQ로 올리는 조건](Coordinator.md#hq로-올리는-조건)) |
...
> [!NOTE]
> 제안 — 제안서 04 Q3 (V1.0.2 Q6 입력 이월) 결정 대기.

- **EV-341 추적표** — 적용 대상 verification에는 `## 증거 추적` 표를 둔다. 적용 대상은 [관리자 검토 C6](../../README.md#관리자-검토-체크리스트)에서 정한다 (AI 권장: 표준·엄격 깊이의 설계·검증·측정·출판 산출물).
...
- [Planner](Planner.md) — 평가 결과를 받아 계획을 고치는 역할
- [Executor](Executor.md) — 검증할 결과를 만드는 역할
- [제어 설정](../../HQ/Control_Settings.md#예산과-반복-한도) — HQ가 조정하는 한도와 수치
- [작업 흐름](../Workflow.md#결과-검증) — 평가와 검증이 들어가는 단계
