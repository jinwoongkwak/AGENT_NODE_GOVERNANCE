...
AGENT_NODE_GOVERNANCE을 바꾸고, 정본으로 교체하고, 기여하고, 다른 기업에 옮기는 방법입니다. 대상은 [HQ](../Architecture/Operating_Model.md#hq와-ai의-뜻), 기여자, 도입하려는 기업입니다.
...
AGENT_NODE_GOVERNANCE의 운영 규칙 채택은 HQ 결정으로만 바뀝니다. 요청받은 초안 개선·검사·저장소 배포는 운영 채택과 별개이며, 새 규칙은 승인 전까지 초안으로 구분합니다. 확장 사양은 미승인이라는 뜻이 아니라 선택 기능의 적용 범위입니다. 변경은 일반 작업과 같은 [TaskNote](../Architecture/Document_System.md#작업-문서)에서 [결정표](Commands_and_Approval.md#결정표-작성)로 제안하고 승인받습니다.
...
| 4 | [`{legacy-control-folder}`](../Architecture/Company_Profile.md#작업-공간-경로)의 규약을 AGENT_NODE_GOVERNANCE 해당 섹션으로 가는 안내 문서로 교체 | 규칙 문장 중복 0 |
